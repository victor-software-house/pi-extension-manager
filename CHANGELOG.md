...
- **Pi 0.75 managed-root resolution.** `resolveNpmPackageRoot` (`src/packages/extensions.ts`) now prefers `~/.pi/agent/npm/node_modules/<pkg>` (the new Pi 0.75 user-scope install root, closes earendil-works/pi#4587) over the legacy `npm root -g` location. Before this change, the legacy global root was checked first, so on machines where `pi update` has migrated extensions to the managed root but the old copies under `/opt/homebrew/lib/node_modules/` (or equivalent) are still present, `/extensions` resolved the stale legacy path and reported outdated versions. The legacy global root is still consulted as a fallback for packages that have not been migrated yet.
- **Canonical `getAgentDir()` for the managed-root prefix.** The hand-rolled `process.env.PI_PACKAGE_DIR || ~/.pi/agent` fallback now goes through `getAgentDir()` from `@earendil-works/pi-coding-agent`, so the resolver honors `PI_CODING_AGENT_DIR` exactly the way Pi's own package manager does. `PI_PACKAGE_DIR` is preserved as a non-standard pre-0.75 escape hatch.
- **CI: `NODE_AUTH_TOKEN` falls back to `github.token`.** `release.yml` and `ci.yml` now use `secrets.GH_PACKAGES_TOKEN || github.token`, matching the rest of the `@victor-software-house` Pi ecosystem. Combined with the package's Manage Actions Access grant, the workflow `github.token` is sufficient to publish; no dedicated PAT required.

### Changed

- **Declare Pi 0.75+ support.** Bump `@earendil-works/{pi-coding-agent,pi-tui}` peer dependency ranges from `>=0.74.0` to `>=0.75.0`. Real `@earendil-works/{pi-coding-agent,pi-tui}@^0.75.0` added to `devDependencies` so local typecheck runs against the actual published Pi 0.75 type surfaces instead of the previous implicit shims.

## 0.8.3 — 2026-04-08

_Auto-generated by release-please, no explicit notes._
