...
import type { InstalledPackage, State, UnifiedAction, UnifiedItem } from "../types/index.js";
...
import { formatSize, getChangeMarker, getPackageIcon, getScopeIcon, getStatusIcon } from "./theme.js";

async function showInteractiveFallback(ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
	await showListOnly(ctx);
	await showInstalledPackagesList(ctx, pi);
}

export async function showInteractive(ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
	if (!hasCustomUI(ctx)) {
		notify(
			ctx,
...
async function showInteractiveOnce(ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<boolean> {
	const initialData = await runTaskWithLoader(
		...
		async ({ signal, setMessage }) => {
			...
			const [localEntries, installedPackages] = await Promise.all([localEntriesPromise, installedPackagesPromise]);
		...
		() =>
			ctx.ui.custom<UnifiedAction>((tui, theme, _keybindings, done) => {
				...
				const syncThemedContent = (): void => {
					...
					quickText.setText(theme.fg("dim", "Quick: i Install | f Search | U Update all | t Auto-update | p Palette"));
...
function normalizePathForDuplicateCheck(value: string): string {
	...
	const looksWindowsPath = /^[a-zA-Z]:\//.test(normalized) || normalized.startsWith("//") || value.includes("\\");
...
export function buildUnifiedItems(
	...
	for (const pkg of installedPackages) {
		...
		const pkgResolvedNormalized = pkg.resolvedPath ? normalizePathForDuplicateCheck(pkg.resolvedPath) : "";
		...
		for (const localPath of localPaths) {
			...
			if (
				...
				(localPath.startsWith(`${pkgResolvedNormalized}/`) || pkgResolvedNormalized.startsWith(localPath))
			) {
				isDuplicate = true;
				break;
			}
...
function buildSettingsItems(items: UnifiedItem[], staged: Map<string, State>, theme: Theme): SettingItem[] {
...
function formatUnifiedItemLabel(item: UnifiedItem, state: State, theme: Theme, changed = false): string {
	if (item.type === "local") {
...
async function applyToggleChangesFromManager(
	...
	if (apply.changed > 0) {
		...
		if (shouldPromptReload) {
			const shouldReload = await ctx.ui.confirm("Reload Required", "Local extensions changed. Reload pi now?");
	...
	switch (destination) {
		...
		case "auto-update":
			await promptAutoUpdateWizard(pi, ctx, (packages) => {
				ctx.ui.notify(`Updates available for ${packages.length} package(s): ${packages.join(", ")}`, "info");
			});
...
async function handleUnifiedAction(
	...
	if (result.type === "action") {
		...
		const pending = await resolvePendingChangesBeforeLeave(items, staged, byId, ctx, pi, pendingDestination);
		if (pending === "stay") return false;
		if (pending === "exit") return true;
		...
		if (item.type === "local") {
			...
			ctx.ui.notify(`Removed ${item.displayName}${removal.removedDirectory ? " (directory)" : ""}.`, "info");
		...
		const selection =
			!result.action || result.action === "menu" ? await promptPackageActionSelection(pkg, ctx) : result.action;
...
async function applyStagedChanges(items: UnifiedItem[], staged: Map<string, State>, pi: ExtensionAPI) {
	let changed = 0;
	const errors: string[] = [];
	...
	for (const item of items) {
		...
		const result = await setExtensionState({ activePath: item.activePath, disabledPath: item.disabledPath }, target);
...
// Legacy redirect
export async function showInstalledPackagesLegacy(ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<void> {
	if (!hasCustomUI(ctx)) {
		await showInstalledPackagesList(ctx, pi);
		return;
	}

	ctx.ui.notify("Use /ext for the unified view.\nInstalled packages are now shown alongside local extensions.", "info");
