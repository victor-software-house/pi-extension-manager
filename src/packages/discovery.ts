...
async function fetchNpmSearchPage(
	...
	const response = await fetchWithTimeout(`${NPM_SEARCH_API}?${params.toString()}`, TIMEOUTS.npmSearch, signal);
	...
	return {
		total: typeof data.total === "number" && Number.isFinite(data.total) ? data.total : packages.length,
		resultCount: objects.length,
		packages,
	};
}

export async function fetchNpmRegistrySearchResults(query: string, signal?: AbortSignal): Promise<NpmPackage[]> {
...
function getInstalledPackageIdentity(pkg: InstalledPackage): string {
	return normalizePackageIdentity(pkg.source, pkg.resolvedPath ? { resolvedPath: pkg.resolvedPath } : undefined);
}
...
async function hydratePackageFromResolvedPath(pkg: InstalledPackage): Promise<void> {
	...
	try {
		...
		if (!pkg.description && typeof manifest.description === "string" && manifest.description.trim()) {
			pkg.description = manifest.description.trim();
		}

		if ((!pkg.name || pkg.name === pkg.source) && typeof manifest.name === "string" && manifest.name.trim()) {
