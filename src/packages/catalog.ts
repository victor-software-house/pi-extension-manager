...
type PiPackageUpdate = Awaited<ReturnType<DefaultPackageManager["checkForAvailableUpdates"]>>[number];
...
function createPackageRecord(source: string, scope: PiScope, packageManager: DefaultPackageManager): InstalledPackage {
...
function setProgressCallback(packageManager: DefaultPackageManager, onProgress?: (event: ProgressEvent) => void): void {
...
function createDefaultPackageCatalog(cwd: string): PackageCatalog {
	...
	return {
		listInstalledPackages(options) {
			...
			return Promise.resolve(options?.dedupe === false ? installed : dedupeInstalledPackages(installed));
		},
