	...
	await Promise.allSettled(
		paths.map(async (path) => {
			try {
				...
				console.warn(`[extmgr] Failed to remove temporary standalone install artifact at ${path}:`, error);
			}
		}),
	);
}
...
export async function installPackage(
	...
	const githubTsMatch = source.match(/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/([^/]+)\/(.+\.ts)$/);
...
export async function installPackageLocally(
	...
	// Extract
	const extractDir = join(tempDir, `extracted-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
	...
	const extractSuccess = await tryOperation(
		...
		async () => {
			...
			const extractRes = await pi.exec("tar", ["-xzf", tarballPath, "-C", extractDir, "--strip-components=1"], {
				timeout: TIMEOUTS.extractPackage,
				cwd: ctx.cwd,
			});
