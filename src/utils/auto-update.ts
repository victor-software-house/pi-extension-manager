...
export function startAutoUpdateTimer(
	...
	const initialDelayMs = typeof nextCheck === "number" && nextCheck > now ? Math.max(0, nextCheck - now) : 0;
 ...
 * Disable auto-update
 */
export function disableAutoUpdate(pi: ExtensionAPI, ctx: ExtensionCommandContext | ExtensionContext): void {
	stopAutoUpdateTimer();
