 ...
 */
export async function confirmReload(ctx: ExtensionCommandContext, reason: string): Promise<boolean> {
	if (!ctx.hasUI) {
 ...
 */
export function formatListOutput(ctx: ExtensionCommandContext, title: string, items: string[]): void {
	if (items.length === 0) {
