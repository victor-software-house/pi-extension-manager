...
export function normalizeLocalSourceIdentity(source: string): string {
	...
	const looksWindowsPath = /^[a-zA-Z]:\//.test(normalized) || normalized.startsWith("//") || source.includes("\\");
...
export function normalizePackageIdentity(source: string, options?: { resolvedPath?: string }): string {
	const normalized = sanitizeSource(source);
